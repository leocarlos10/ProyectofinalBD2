...
export const TableComponent = ({ pagina }) => {
  const { ListaCitas, getCitas, loading, error, eliminarCita, ActualizarCita } =
    useContext(CitaContext);
  const { listaPacientes, getPacientes, loadingP, errorP } =
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const estaVacio = Object.values(newDiagnostico).some(
  ...
  const filteredCitas = ListaCitas.filter(
    (object) =>
      object.usuario.nombre.toLowerCase().includes(lowercasedSearchTerm) ||
      (object.cita.servicio &&
        object.cita.servicio.toLowerCase().includes(lowercasedSearchTerm)) ||
      object.cita.estado.toLowerCase().includes(lowercasedSearchTerm)
  );

  const filteredPacientes = listaPacientes.filter(
    (paciente) =>
  ...
  const filteredDiagnosticos = diagnosticos.filter(
    (object) =>
      (object.usuario.nombre &&
        object.usuario.nombre.toLowerCase().includes(lowercasedSearchTerm)) ||
      (object.diagnostico.observacion &&
        object.diagnostico.observacion
          .toLowerCase()
          .includes(lowercasedSearchTerm)) ||
      (object.diagnostico.tratamiento &&
        object.diagnostico.tratamiento
          .toLowerCase()
          .includes(lowercasedSearchTerm))
  );
  ...
  const contenido_diagnosticos = filteredDiagnosticos.map((object) => {
    return (
      <DiagnosticoTable
        id_diagnostico={object.diagnostico.id_diagnostico}
        object={object}
  ...
  return (
    <div className="px-8 pb-8">
      ...
      <div className="flex items-center gap-4 mb-6">
        <div className="flex items-center bg-[#f5f6fa] rounded-lg px-4 py-2 w-full max-w-xl">
          <svg
        ...
        {pagina === "diagnosticos" && (
          <button
            className="bg-secondary text-white px-4 py-2 rounded-md hover:bg-secondary/90 transition-colors cursor-pointer whitespace-nowrap"
            onClick={abrirModal}
          >
            + Nuevo Diagnóstico
          </button>
        ...
        >
          <div className="flex flex-col gap-4">
            ...
            <form className="space-y-2">
              <div>
                ...
                >
                  ...
                  {listaPacientes.map((paciente) => (
                    <option key={paciente.id_paciente} value={paciente.cedula}>
              ...
              <div>
                <label className="block text-gray-700 mb-1" htmlFor="notaCorta">
                  Descripción Corta
                </label>
                <input
                  name="notaCorta"
              ...
              <div>
                <label className="block text-gray-700 mb-1" htmlFor="notaLarga">
                  Descripción Larga
                </label>
                <textarea
                  name="notaLarga"
      ...
      <table className="min-w-full table-fixed divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr className="border-b">
            {pagina === "citas" ? (
              <>
                <th className="w-1/5 py-3 px-4 font-medium text-gray-700">
                  Nombre del Paciente
                </th>
                <th className="w-1/5 py-3 px-4 font-medium text-gray-700">
                  Fecha y hora
              ...
              pagina === "diagnosticos" && (
                <>
                  <th className="w-1/6 py-3 px-4 font-medium text-gray-700 text-center">
                    Paciente
                  </th>
                  <th className="w-1/6 py-3 px-4 font-medium text-gray-700">
                    Observacion
                  </th>
                  <th className="w-1/6 py-3 px-4 font-medium text-gray-700">
                    Tratamiento
